...
tid_t process_create_initd(const char *file_name) {
  ...
  //  fork는 아니지만 예외적으로 유저프로그램을 실행하는 쓰레드를 main쓰레드의
  //  자식으로 설정
...
static void __do_fork(void *aux) {
  ...
  */
  ...
  // struct list *child_list = &parent->child_list;
  // struct child_status* ch_st = NULL;
  // for(struct list_elem *e = list_begin(child_list); e !=
  // list_end(child_list); e = list_next(e)){
  // 	//printf("for tid: %d\n", list_entry(e, struct child_status,
  // elem)->tid); 	if(list_entry(e, struct child_status, elem)->tid ==
  // current->tid){ 		ch_st = list_entry(e, struct child_status,
  // elem);
  // 	}
  // }
  // //lock_release(&current->childlist_lock);
 ...
 * Returns -1 on fail. */
// 현재 실행 중인 프로세스(쓰레드)의 실행 이미지를 새로운 유저 프로그램으로
// 바꾸는 함수 start_process()
...
// ELF 실행 파일의 프로그램 헤더가 유효하고 메모리에 적재할 수 있는 세그먼트인지
// 검사
...
static bool setup_stack(struct intr_frame *if_) {
  ...
  if (kpage != NULL) {
    ...
    // 커널에서 할당한 물리 페이지를 사용자 주소 공간의 스택 가장 위에 해당하는
    // 가상주소와 매핑 uint8_t 포인터로 캐스팅한 이유는 바이트단위의 연산을
    // 위함. 스택은 거꾸로 자라기 때문에 스택포인터에서 - 페이지사이즈만큼 간
    // 주소에 할당한 1페이지 물리주소 연결
...
static bool lazy_load_segment(struct page *page, void *aux) {
  ...
  off_t offset = ((struct file_info *)aux)->offset;
  size_t page_read_bytes = ((struct file_info *)aux)->page_read_bytes;
  size_t page_zero_bytes = PGSIZE - page_read_bytes;
  file_seek(file, offset); // file의 오프셋을 offset으로 바꾼다. 이제 offset부터
                           // 읽기 시작한다.

  /* 페이지에 매핑된 물리 메모리(frame, 커널 가상 주소)에 파일의 데이터를
   * 읽어온다. */
  ...
  // // position을 offset으 지정
  // file_seek(file_info->file, file_info->offset);

  // if (file_read(file_info->file, page->frame->kva,
  //               file_info->page_read_bytes) !=
  //     (int)(file_info->page_read_bytes)) {
  //   palloc_free_page(page->frame->kva);
  //   return false;
  // }
  // memset(page->frame->kva + file_info->page_read_bytes, 0,
