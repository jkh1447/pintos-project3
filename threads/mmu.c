 ...
 * pointer is returned. */
//  va라는 가상 주소에 대응하는 최종 페이지 테이블 엔트리(PTE)의 주소를
//  리턴합니다.
uint64_t *pml4e_walk(uint64_t *pml4e, const uint64_t va, int create) {
  ...
  if (pml4e) {
    /* 가상 주소에서 추출한 pml4테이블의 인덱스에 접근해서,
            ...
            그래서 이후 ptov (PTE_ADDR (pml4e[idx]) 이 부분처럼 가상주소로
       변환해서 사용한다.
    */
...
/* 	이 페이지 테이블을 CPU가 사용하는 활성 페이지 테이블로 설정합니다.
        CPU는 현재 활성화된 페이지 테이블을 기반으로 가상 주소 → 물리 주소를
   변환합니다. 내부적으로는 CR3 레지스터에 물리 주소를 넣어서 페이지 테이블을
   바꿉니다.
*/
...
/*
        ...
        upage는 이미 연결되어있으면 안되고, kpage는 palloc_get_page()함수로
   할당된 커널 주소이여야 함.
*/
bool pml4_set_page(uint64_t *pml4, void *upage, void *kpage, bool rw) {
  ...
  if (pte)
    ...
    // 그리고 하위 3비트를 or 연산으로 설정해준 뒤 페이지 테이블 엔트리를
    // 설정한다.
